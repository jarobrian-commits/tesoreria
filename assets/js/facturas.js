...
$(document).ready(function () {
  ...
  window.open(`/tesoreria/factura_nueva.php?cliente=${encodeURIComponent(cliente)}`, "_blank");
});
...
const cliente = new URLSearchParams(window.location.search).get("cliente") || "demo";
      window.location.href = `/tesoreria/tesoreria/index.php?cliente=${encodeURIComponent(cliente)}&tab=facturas`;
    } catch (e) {
      alertErr(e);
    }
  });
...
$(document).on("click", ".fac-pdf", function(){
  ...
  const url = "/tesoreria/uploads/facturas/" + file.replace(/^.*[\\/]/,'') + "?t=" + Date.now();
